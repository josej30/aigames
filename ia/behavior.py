...
def getSteering(targets,target,agent,obs,ts,food,tg,mm):
    ...
    if flag == "Wander":
        ...
        if agent.life <= agent.maxlife*0.25 and len(food) > 0:
            ...
            path = pathfindAStar(agent, foodAgent, ts, tg, mm)

            for p in path:         
                tg.t[p.fromNode.node][p.toNode.node] = 999
        ...
        # Astar for player
            ...
            for i in range(len(mm.m)):
                for j in range(len(mm.m)):
                    if mm.m[i][j] != -1 and mm.m[i][j] != 20 and mm.m[i][j] <= coloto:
                        coloto = mm.m[i][j]
                        esposito = j

            for t in ts:
                if t.node == esposito:
                    coloto = t.centerOfMass()

            fantastic = Agent()
            fantastic.position = [coloto[0],0,coloto[1]]

            path = pathfindAStar(agent, fantastic, ts, tg, mm)
